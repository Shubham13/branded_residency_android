...
public class AwsUtill extends AbstractAWSSigner
     ...
     * that want to suppress this, they should use new AwsUtillNew(false).
     */
    ...
    protected String extractServiceName(URI endpoint) {
        ...
        // using the AwsUtillNew directly and not setting a service name
        // explicitly.
    ...
    protected String getCanonicalRequest(Request<?> request, String contentSha256) {
        ...
        StringBuffer sb= new StringBuffer(request.getEndpoint().getPath());

        if(request.getEndpoint().getPath().endsWith("/")){
            sb.deleteCharAt(sb.length()-1);
        }
        String removeLastSlash = sb.toString();
        final String path = HttpUtils.appendUri(removeLastSlash,
        ...
        final String canonicalRequest =
                request.getHttpMethod().toString() + "\n" +
                         ...
                         * path
                         */
                        getCanonicalizedResourcePath(removeLastSlash, doubleUrlEncode) + "\n" +
