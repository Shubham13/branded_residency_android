...
public class CountryPicker implements BottomSheetInteractionListener, LifecycleObserver {
    ...
    @Override public void setupRecyclerView(View sheetView) {
        ...
        adapter = new CountriesAdapter(sheetView.getContext(), searchResults, new OnItemClickListener() {
    ...
    public String getCountryCodeFromSim(String countryName){
        String dialCode = "+91";
        Collections.sort(countries, new DialCodeComparator());
        Country country = new Country();
        country.setCode(countryName.toUpperCase());
        for(int i=0;i<countries.size();i++) {
            if (countries.get(i).getCode().equalsIgnoreCase(country.getCode())) {
                dialCode =  countries.get(i).getDialCode();
            }
        }
        return dialCode;
    }
    ...
    public String getCountryByDialCode(@NonNull String dialCode){
        Collections.sort(countries, new DialCodeComparator());
        Country country = new Country();
        ...
        if (i < 0) {
            ...
            return countries.get(i).getCode();
        }
    }
    // endregion
